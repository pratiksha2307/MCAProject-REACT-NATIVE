...
export default class Agriculture extends React.Component {
  ...
  checkagriculturedata = () => {
    ...
    postData(check, { name: 'Agriculture' })
      .then(async (data) => {
        this.setState({ subagriculture: data });
      })
  }
  check = (data) => {
    if (data == 'agriculture census') {
      console.log(data);
      ...
      <View style={{ flex: 1, backgroundColor: backgroundcolor }}>
        ...
        <ScrollView>
          <View style={{ flex: 1, alignItems: "center", justifyContent: 'center', margin: 25, }}>
            {
              this.state.subagriculture.map((item, key) => (
                <TouchableOpacity
                  key={key}
                  activeOpacity={0}
                  delayPressIn={0}
                    ...
                    <View
                      ...
                      <View style={{ width: '50%', alignItems: 'center', justifyContent: 'center' }}>
                        <Image style={{ height: 80, width: 80, justifyContent: 'center' }}
                          source={{ uri: `http://192.168.43.233:4000/photo/Agriculture/${item.image}` }}
                        />
                      </View>
                      <View style={{ width: '50%', alignItems: 'center', justifyContent: 'center' }}>
                        <Text style={{
