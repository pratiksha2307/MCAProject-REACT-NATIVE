...
export default class EducationJob extends React.Component {
  constructor(props) {
    super(props);
    this.state = {
      result: false,
  ...
  educationjob  = () => {
    ...
    postData(educationjob, { name: 'EducationJob' })
      .then(async (data) => {
        this.setState({ edujob: data });
      })
  }
      ...
      <View style={{ flex: 1, backgroundColor: "#ffffff" }}>
        ...
        <ScrollView>
          <View style={{ flex: 1, alignItems: "center", justifyContent: 'center', margin: 25, }}>
            {
              this.state.edujob.map((item, key) => (
                <View key={key}>
