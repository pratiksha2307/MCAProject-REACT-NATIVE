...
export default class EducationPhd extends React.Component {
  constructor(props) {
    super(props);
    this.state = {
      result: false,
  ...
  educationphd  = () => {
    ...
    postData(educationphd, { name: 'EducationPhd' })
      .then(async (data) => {
        this.setState({ eduphd: data });
      })
  }
      ...
      <View style={{ flex: 1, backgroundColor: "#ffffff" }}>
        ...
        <ScrollView>
          <View style={{ flex: 1, alignItems: "center", justifyContent: 'center', margin: 25, }}>
            {
              this.state.eduphd.map((item, key) => (
                <View key={key}>
