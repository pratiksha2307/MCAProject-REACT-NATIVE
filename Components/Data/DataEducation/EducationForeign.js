...
export default class EducationForeign extends React.Component {
  constructor(props) {
    super(props);
    this.state = {
      result: false,
  ...
  educationforeign  = () => {
    ...
    postData(educationforeign, { name: 'EducationForeign' })
      .then(async (data) => {
        this.setState({ edufor: data });
      })
  }
      ...
      <View style={{ flex: 1, backgroundColor: "#ffffff" }}>
        ...
        <ScrollView>
          <View style={{ flex: 1, alignItems: "center", justifyContent: 'center', margin: 25, }}>
            {
              this.state.edufor.map((item, key) => (
                <View key={key}>
