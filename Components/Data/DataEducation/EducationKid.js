...
export default class EducationKid extends React.Component {
  constructor(props) {
    super(props);
    this.state = {
      result: false,
  ...
  educationkid = () => {
    ...
    postData(educationkid, { name: 'EducationKid' })
      .then(async (data) => {
        this.setState({ edukid: data });
      })
  }
      ...
      <View style={{ flex: 1, backgroundColor: "#ffffff" }}>
        ...
        <ScrollView>
          <View style={{ flex: 1, alignItems: "center", justifyContent: 'center', margin: 25, }}>
            {
              this.state.edukid.map((item, key) => (
                <View key={key}>
