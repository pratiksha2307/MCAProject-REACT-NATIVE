...
export default class EducationHigher extends React.Component {
  constructor(props) {
    super(props);
    this.state = {
      result: false,
  ...
  educationhigher = () => {
    ...
    postData(educationhigher, { name: 'Educationhigher' })
      .then(async (data) => {
        this.setState({ eduhig: data });
      })
  }
    ...
    return (
      <View style={{ flex: 1, backgroundColor: "#ffffff" }}>
        ...
        <ScrollView>
          ...
          {
              this.state.eduhig.map((item, key) => (
                <View key={key}>
