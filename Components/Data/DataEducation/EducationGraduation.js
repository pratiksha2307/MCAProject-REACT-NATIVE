...
export default class EducationGraduation extends React.Component {
  constructor(props) {
    super(props);
    this.state = {
      result: false,
  ...
  educationgraduation = () => {
    ...
    postData(educationgraduation, { name: 'EducationGraduation' })
      .then(async (data) => {
        this.setState({ edugra: data });
      })
  }
      ...
      <View style={{ flex: 1, backgroundColor: "#ffffff" }}>
        ...
        <ScrollView>
          <View style={{ flex: 1, alignItems: "center", justifyContent: 'center', margin: 25, }}>
            {
              this.state.edugra.map((item, key) => (
                <View key={key}>
