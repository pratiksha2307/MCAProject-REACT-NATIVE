...
export default class EducationPostGraduation extends React.Component {
  constructor(props) {
    super(props);
    this.state = {
      result: false,
  ...
  educationpostgraduation  = () => {
    ...
    postData(educationpostgraduation, { name: 'EducationPostGraduation' })
      .then(async (data) => {
        this.setState({ edupos: data });
      })
  }
      ...
      <View style={{ flex: 1, backgroundColor: "#ffffff" }}>
        ...
        <ScrollView>
          <View style={{ flex: 1, alignItems: "center", justifyContent: 'center', margin: 25, }}>
            {
              this.state.edupos.map((item, key) => (
                <View key={key}>
