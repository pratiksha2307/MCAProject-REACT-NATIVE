...
export default class HealthBrain extends React.Component {
  constructor(props) {
    super(props);
    this.state = {
      result: false,
  ...
  healthbrain = () => {
    ...
    postData(healthbrain, { name: 'HealthBrain' })
      .then(async (data) => {
        this.setState({ healbra: data });
      })
  }
      ...
      <View style={{ flex: 1, backgroundColor: "#ffffff" }}>
        ...
        <ScrollView>
          <View style={{ flex: 1, alignItems: "center", justifyContent: 'center', margin: 25, }}>
            {
              this.state.healbra.map((item, key) => (
                <View key={key}>
