...
export default class HealthCancer extends React.Component {
  constructor(props) {
    super(props);
    this.state = {
      result: false,
  ...
  healthcancer = () => {
    ...
    postData(healthcancer, { name: 'HealthCancer' })
      .then(async (data) => {
        this.setState({ healcan: data });
      })
  }
      ...
      <View style={{ flex: 1, backgroundColor: "#ffffff" }}>
        ...
        <ScrollView>
          <View style={{ flex: 1, alignItems: "center", justifyContent: 'center', margin: 25, }}>
            {
              this.state.healcan.map((item, key) => (
                <View key={key}>
