...
export default class HealthKidney extends React.Component {
  constructor(props) {
    super(props);
    this.state = {
      result: false,
  ...
  healthkidney = () => {
    ...
    postData(healthkidney, { name: 'HealthKidney' })
      .then(async (data) => {
        this.setState({ healkid: data });
      })
  }
      ...
      <View style={{ flex: 1, backgroundColor: "#ffffff" }}>
        ...
        <ScrollView>
          <View style={{ flex: 1, alignItems: "center", justifyContent: 'center', margin: 25, }}>
            {
              this.state.healkid.map((item, key) => (
                <View key={key}>
