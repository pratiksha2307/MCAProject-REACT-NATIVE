...
export default class HealthHeart extends React.Component {
  constructor(props) {
    super(props);
    this.state = {
      result: false,
  ...
  healthheart = () => {
    ...
    postData(healthheart, { name: 'HealthHeart' })
      .then(async (data) => {
        this.setState({ healhea: data });
      })
  }
      ...
      <View style={{ flex: 1, backgroundColor: "#ffffff" }}>
        ...
        <ScrollView>
          <View style={{ flex: 1, alignItems: "center", justifyContent: 'center', margin: 25, }}>
            {
              this.state.healhea.map((item, key) => (
                <View key={key}>
