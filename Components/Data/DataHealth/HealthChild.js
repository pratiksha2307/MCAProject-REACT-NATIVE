...
export default class HealthChild extends React.Component {
  constructor(props) {
    super(props);
    this.state = {
      result: false,
  ...
  healthchild = () => {
    ...
    postData(healthchild, { name: 'HealthChild' })
      .then(async (data) => {
        this.setState({ healchi: data });
      })
  }
      ...
      <View style={{ flex: 1, backgroundColor: "#ffffff" }}>
        ...
        <ScrollView>
          <View style={{ flex: 1, alignItems: "center", justifyContent: 'center', margin: 25, }}>
            {
              this.state.healchi.map((item, key) => (
                <View key={key}>
