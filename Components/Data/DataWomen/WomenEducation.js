...
export default class WomenEducation extends React.Component {
  constructor(props) {
    super(props);
    this.state = {
      result: false,
  ...
  womeneducation = () => {
    ...
    postData(subcategory, { name: 'WomenEducation' })
      .then(async (data) => {
        this.setState({ womenedu: data });
      })
  }
      ...
      <View style={{ flex: 1, backgroundColor: "#ffffff" }}>
        ...
        <ScrollView>
          <View style={{ flex: 1, alignItems: "center", justifyContent: 'center', margin: 25, }}>
            {
              this.state.womenedu.map((item, key) => (
                <View key={key}>
