...
export default class WomenEmployment extends React.Component {
  constructor(props) {
    super(props);
    this.state = {
      result: false,
  ...
  womenemployment = () => {
    ...
    postData(subcategory2, { name: 'WomenEmployment' })
      .then(async (data) => {
        this.setState({ womenemp: data });
      })
  }
      ...
      <View style={{ flex: 1, backgroundColor: "#ffffff" }}>
        ...
        <ScrollView>
          <View style={{ flex: 1, alignItems: "center", justifyContent: 'center', margin: 25, }}>
            {
              this.state.womenemp.map((item, key) => (
                <View key={key}>
