...
export default class SportsBadminton extends React.Component {
  constructor(props) {
    super(props);
    this.state = {
      result: false,
  ...
  sportsbadminton= () => {
    ...
    postData(sportsbadminton, { name: 'SportsBadminton' })
      .then(async (data) => {
        this.setState({ spobad: data });
      })
  }
      ...
      <View style={{ flex: 1, backgroundColor: "#ffffff" }}>
        ...
        <ScrollView>
          <View style={{ flex: 1, alignItems: "center", justifyContent: 'center', margin: 25, }}>
            {
              this.state.spobad.map((item, key) => (
                <View key={key}>
