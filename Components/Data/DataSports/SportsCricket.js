...
export default class SportsCricket extends React.Component {
  constructor(props) {
    super(props);
    this.state = {
      result: false,
  ...
  sportscricket= () => {
    ...
    postData(sportscricket, { name: 'SportsCricket' })
      .then(async (data) => {
        this.setState({ spocri: data });
      })
  }
      ...
      <View style={{ flex: 1, backgroundColor: "#ffffff" }}>
        ...
        <ScrollView>
          <View style={{ flex: 1, alignItems: "center", justifyContent: 'center', margin: 25, }}>
            {
              this.state.spocri.map((item, key) => (
                <View key={key}>
