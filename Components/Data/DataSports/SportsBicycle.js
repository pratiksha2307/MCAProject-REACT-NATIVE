...
export default class SportsBicycle extends React.Component {
  constructor(props) {
    super(props);
    this.state = {
      result: false,
  ...
  sportsbicycle= () => {
    ...
    postData(sportsbicycle, { name: 'SportsBicycle' })
      .then(async (data) => {
        this.setState({ spobic: data });
      })
  }
      ...
      <View style={{ flex: 1, backgroundColor: "#ffffff" }}>
        ...
        <ScrollView>
          <View style={{ flex: 1, alignItems: "center", justifyContent: 'center', margin: 25, }}>
            {
              this.state.spobic.map((item, key) => (
                <View key={key}>
