...
export default class SportsBasketball extends React.Component {
  constructor(props) {
    super(props);
    this.state = {
      result: false,
  ...
  sportsbasketball= () => {
    ...
    postData(sportsbasketball, { name: 'SportsBasketball' })
      .then(async (data) => {
        this.setState({ spobas: data });
      })
  }
      ...
      <View style={{ flex: 1, backgroundColor: "#ffffff" }}>
        ...
        <ScrollView>
          <View style={{ flex: 1, alignItems: "center", justifyContent: 'center', margin: 25, }}>
            {
              this.state.spobas.map((item, key) => (
                <View key={key}>
