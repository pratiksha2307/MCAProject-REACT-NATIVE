...
export default class SportsBoxing extends React.Component {
  constructor(props) {
    super(props);
    this.state = {
      result: false,
  ...
  sportsboxing= () => {
    ...
    postData(sportsboxing, { name: 'SportsBoxing' })
      .then(async (data) => {
        this.setState({ spobox: data });
      })
  }
      ...
      <View style={{ flex: 1, backgroundColor: "#ffffff" }}>
        ...
        <ScrollView>
          <View style={{ flex: 1, alignItems: "center", justifyContent: 'center', margin: 25, }}>
            {
              this.state.spobox.map((item, key) => (
                <View key={key}>
